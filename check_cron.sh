...
CRON_NEZHA="echo | bash <(curl -s https://raw.githubusercontent.com/ansoncloud8/am-serv00-nezha/main/install-agent.sh)"
...
REBOOT_COMMAND="@reboot cd xray && nohup ./xray -c config.json & && $PM2_PATH resurrect >> /home/$(whoami)/pm2_resurrect.log 2>&1"
...
if [ "$(command -v pm2)" == "/home/${USER}/.npm-global/bin/pm2" ]; then
  ...
  if [ -e "${WORKDIR}/start.sh" ] && [ -e "${FILE_PATH}/config.json" ]; then
    ...
    (crontab -l | grep -F "@reboot cd xray && nohup ./xray -c config.json & && ${CRON_NEZHA}") || (crontab -l; echo "@reboot cd xray && nohup ./xray -c config.json & && ${CRON_NEZHA}") | crontab -
    (crontab -l | grep -F "* * pgrep -x \"nezha-agent\" > /dev/null || ${CRON_NEZHA}") || (crontab -l; echo "*/12 * * * * pgrep -x \"nezha-agent\" > /dev/null || ${CRON_NEZHA}") | crontab -
  elif [ -e "${FILE_PATH}/config.json" ]; then
    echo "添加 socks5 的 crontab 重启任务"
    (crontab -l | grep -F "@reboot cd xray && nohup ./xray -c config.json & && ${CRON_S5}") || (crontab -l; echo "@reboot cd xray && nohup ./xray -c config.json & && ${CRON_S5}") | crontab -
    (crontab -l | grep -F "* * pgrep -x \"s5\" > /dev/null || ${CRON_S5}") || (crontab -l; echo "*/12 * * * * pgrep -x \"s5\" > /dev/null || ${CRON_S5}") | crontab -
  fi
fi
